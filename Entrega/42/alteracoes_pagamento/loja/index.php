            ...
            <div class="pgto-detalhes" id="pgto-detalhe-pix">
                <div class="pix-container" id="pix-container">
                    ...
                    <div class="pix-copia-cola">
                        <span id="pix-codigo-texto" style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></span>
...
function selecionarPgto(tipo) {
    ...
    // Para PIX: gera QR Code de prévia
    if (tipo === 'pix') {
        const total = carrinho.reduce((s,i)=>s+i.quantidade*i.preco_venda,0);
        gerarQrCodePix('pix-qr-canvas', null, total);
        document.getElementById('pix-codigo-texto').textContent = '(código gerado após confirmar pedido)';
    }
}

function copiarPix() {
    const txt = document.getElementById('pix-codigo-texto').textContent;
    if (!txt || txt.includes('gerado após')) return;
...
// Gera QR Code usando API pública de QR (qrserver)
function gerarQrCodePix(canvasId, payload, total) {
    ...
    if (!payload) {
        // Prévia sem payload real — exibe placeholder
        ctx.fillStyle = '#f0f0f0';
        ctx.fillRect(0,0,size,size);
        ctx.fillStyle = '#999';
        ctx.font = '12px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('QR Code PIX', size/2, size/2-8);
        ctx.fillText('R$ ' + fmtBRL(total), size/2, size/2+10);
        return;
    }

    // Usa qrcode.js embutido via API de imagem
    ...
    img.onerror = () => {
        ...
        ctx.fillText('Copie o código abaixo', size/2, size/2);
    };
    img.src = url;
}
    ...
    try {
        ...
        if (d.success) {
            ...
            if (_pgtoSelecionado === 'pix' && d.pix_payload) {
                ...
                gerarQrCodePix('pix-qr-canvas-final', d.pix_payload, 0);
