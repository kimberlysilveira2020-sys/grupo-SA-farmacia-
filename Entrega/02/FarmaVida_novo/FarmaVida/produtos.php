...
// Consulta produtos com estoque calculado
$stmt = $pdo->query("
                    ...
                    <span class="d-flex justify-content-between w-100 me-3 align-items-center">
                        <span class="d-flex align-items-center gap-2">
                            ...
                            <img src="uploads/produtos/<?= htmlspecialchars($produto['foto']) ?>" 
                                 ...
                                 style="width:38px; height:38px; object-fit:cover; border-radius:6px; border:1px solid #dee2e6;">
                            <?php else: ?>
                ...
                <div class="accordion-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-2 text-center">
                            ...
                            <img src="uploads/produtos/<?= htmlspecialchars($produto['foto']) ?>"
                                 alt="<?= htmlspecialchars($produto['nome']) ?>"
                                 class="img-thumbnail" style="max-width:110px; max-height:110px; object-fit:cover;">
                            <?php else: ?>
...
<div class="modal fade" id="modalNovoProduto" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form id="formNovoProduto" onsubmit="salvarProduto(event)" enctype="multipart/form-data">
...
<div class="modal fade" id="modalEditarProduto" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form id="formEditarProduto" onsubmit="salvarEdicao(event)" enctype="multipart/form-data">
                <div class="modal-body">
...
<script>
    ...
    async function salvarProduto(event) {
        ...
        try {
            ...
            else { alert('Erro: ' + result.message); btn.disabled=false; btn.innerHTML='<i class="bi bi-save"></i> Salvar Produto'; }
        } catch (error) { alert('Erro ao salvar produto'); btn.disabled=false; btn.innerHTML='<i class="bi bi-save"></i> Salvar Produto'; }
    }
    ...
    async function salvarEdicao(event) {
        ...
        const formData = new FormData(event.target);
        try {
            const response = await fetch('api.php?endpoint=produtos_editar_form', { method: 'POST', body: formData });
            const result = await response.json();
            if (result.success) location.reload();
            else { alert('Erro: ' + result.message); btn.disabled=false; btn.innerHTML='<i class="bi bi-save"></i> Salvar Alterações'; }
        } catch (error) { alert('Erro ao atualizar produto'); btn.disabled=false; btn.innerHTML='<i class="bi bi-save"></i> Salvar Alterações'; }
    }
    ...
    // Botão editar: preenche modal
    document.querySelectorAll('.btn-editar-produto').forEach(btn => {
        btn.addEventListener('click', function() {
            document.getElementById('edit_produto_id').value  = this.dataset.id;
            document.getElementById('edit_nome').value        = this.dataset.nome;
            document.getElementById('edit_fabricante').value  = this.dataset.fabricante;
            document.getElementById('edit_categoria').value   = this.dataset.categoria;
            document.getElementById('edit_preco_venda').value = this.dataset.preco;
    ...
    document.querySelectorAll('.btn-adicionar-lote').forEach(btn => {
        btn.addEventListener('click', function() {
            document.getElementById('produto_id_lote').value    = this.dataset.produtoId;
            document.getElementById('produto_nome_display').value = this.dataset.produtoNome;
    ...
    document.querySelectorAll('.btn-deletar-produto').forEach(btn => {
        btn.addEventListener('click', function() { deletarProduto(this.dataset.id); });
    });

    // Carrega lotes ao expandir accordion
    document.querySelectorAll('.accordion-button').forEach(btn => {
        btn.addEventListener('click', async function() {
            const target = this.getAttribute('data-bs-target');
            const produtoId = target.replace('#collapse-', '');
            const tabela = document.querySelector(`#tabela-lotes-${produtoId} tbody`);

            if (tabela.dataset.loaded) return;
