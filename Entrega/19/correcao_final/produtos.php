...
<script>
    ...
    document.getElementById('formNovoProduto').addEventListener('submit', async function(event) {
        ...
        if (btn.disabled) return; // evita duplo disparo
        btn.disabled = true;
        btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Salvando...';
        try {
            const res    = await fetch('api.php?endpoint=produtos_criar', { method: 'POST', body: new FormData(this) });
    ...
    // ── Carrega lotes ao expandir + garante só 1 aberto por vez ──────
    const accordionEl = document.getElementById('accordionProdutos');

    accordionEl.addEventListener('show.bs.collapse', async function(e) {
        // Fecha todos os outros painéis abertos
        accordionEl.querySelectorAll('.accordion-collapse.show').forEach(openEl => {
            if (openEl !== e.target) {
                bootstrap.Collapse.getOrCreateInstance(openEl).hide();
            }
        });

        // Carrega lotes (só uma vez por painel)
        const produtoId = e.target.id.replace('collapse-', '');
