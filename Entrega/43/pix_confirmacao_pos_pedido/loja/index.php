...
/* ══════════════════════════════════════════════════════════
   MODAL PAGAMENTO
══════════════════════════════════════════════════════════ */
.pgto-opcoes { display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:20px; }
.pgto-btn {
    display:flex; flex-direction:column; align-items:center; justify-content:center;
    gap:6px; padding:16px 10px; border:2.5px solid var(--cinza-bord);
    border-radius:12px; background:#fff; cursor:pointer;
    font-size:.8rem; font-weight:700; color:var(--texto);
    transition:all .18s; user-select:none;
}
.pgto-btn i { font-size:1.6rem; }
.pgto-btn:hover { border-color:var(--verde); color:var(--verde); background:#f0faf4; }
.pgto-btn.ativo { border-color:var(--verde); background:var(--verde); color:#fff; }
.pgto-detalhes { display:none; }
.pgto-detalhes.visivel { display:block; }

/* PIX QR Code */
.pix-container { text-align:center; padding:10px 0; }
.pix-qr-wrap {
    display:inline-block; background:#fff; border:3px solid var(--verde);
    border-radius:14px; padding:12px; margin:10px 0;
}
.pix-qr-wrap canvas { display:block; }
.pix-copia-cola {
    display:flex; align-items:center; gap:8px;
    background:#f5f5f5; border-radius:8px; padding:10px 12px;
    font-size:.72rem; word-break:break-all; color:#444;
    margin:10px 0; text-align:left;
}
.pix-copia-cola button {
    flex-shrink:0; background:var(--verde); color:#fff;
    border:none; border-radius:6px; padding:6px 10px;
    font-size:.75rem; font-weight:700; cursor:pointer; white-space:nowrap;
}
.pix-instrucoes { font-size:.78rem; color:var(--texto-sub); line-height:1.5; }
.pix-instrucoes li { margin-bottom:4px; }

/* Crédito */
.credito-info { background:#f0faf4; border-radius:10px; padding:16px; font-size:.82rem; }
.credito-info p { margin:4px 0; }
.credito-aviso { color:#e53935; font-size:.77rem; margin-top:8px; }

/* PayPal */
.paypal-info { text-align:center; padding:10px 0; }
.paypal-info .paypal-icon { font-size:3rem; color:#003087; margin-bottom:8px; }
.paypal-email { font-weight:700; font-size:.9rem; color:#003087; }

/* Boleto */
.boleto-info { background:#fff9e6; border-radius:10px; padding:16px; font-size:.82rem; }
.boleto-info p { margin:4px 0; }
...
<div class="modal-overlay" id="modal-pagamento">
    <div class="modal-box" style="max-width:480px;">
        ...
        <div class="modal-box__body">
            <p style="font-size:.82rem;color:var(--texto-sub);margin-bottom:14px;">
                Total do pedido: <strong id="pgto-total-display" style="color:var(--verde);font-size:1rem;">R$ 0,00</strong>
            </p>

            <!-- Opções de pagamento -->
            <div class="pgto-opcoes">
                <button class="pgto-btn" data-pgto="credito" onclick="selecionarPgto('credito')">
                    <i class="bi bi-credit-card-fill"></i>
                    Crédito
                </button>
                <button class="pgto-btn" data-pgto="pix" onclick="selecionarPgto('pix')">
                    <i class="bi bi-qr-code"></i>
                    PIX
                </button>
                <button class="pgto-btn" data-pgto="paypal" onclick="selecionarPgto('paypal')">
                    <i class="bi bi-paypal"></i>
                    PayPal
                </button>
                <button class="pgto-btn" data-pgto="boleto" onclick="selecionarPgto('boleto')">
            ...
            <!-- Detalhes: Crédito -->
            <div class="pgto-detalhes" id="pgto-detalhe-credito">
                <div class="credito-info">
                    <p><i class="bi bi-info-circle"></i> Parcelamento disponível em até <strong>12x</strong> sem juros (sujeito a aprovação).</p>
                    <p style="margin-top:8px;">Os dados do cartão serão solicitados na próxima etapa com ambiente seguro.</p>
                    <p class="credito-aviso"><i class="bi bi-shield-lock"></i> Pagamento processado com criptografia SSL.</p>
                </div>
            </div>

            <!-- Detalhes: PIX -->
            <div class="pgto-detalhes" id="pgto-detalhe-pix">
                <div class="pix-container" id="pix-container">
                    <p style="font-size:.82rem;color:var(--texto-sub);margin-bottom:10px;">
                    ...
                    <div class="pix-copia-cola">
                        <span id="pix-codigo-texto" style="flex:1;font-size:.68rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:220px;"></span>
                        <button onclick="copiarPix()"><i class="bi bi-clipboard"></i> Copiar</button>
                    </div>
                    <p style="font-size:.75rem;color:var(--texto-sub);margin:6px 0 10px;">
                        <i class="bi bi-key-fill" style="color:var(--verde);"></i>
                        Chave PIX: <strong><?= htmlspecialchars(Config::PIX_CHAVE) ?></strong>
                    </p>
                    <ul class="pix-instrucoes" style="list-style:none;padding:0;text-align:left;">
                        <li>📱 Abra seu banco no celular</li>
                        <li>🔍 Vá em <strong>Pagar via PIX &rarr; QR Code</strong></li>
                        <li>📷 Escaneie o código ou cole o código copiado</li>
                        <li>✅ Confirme o pagamento e clique em <strong>Confirmar Pedido</strong></li>
                    </ul>
                </div>
            </div>

            <!-- Detalhes: PayPal -->
            <div class="pgto-detalhes" id="pgto-detalhe-paypal">
                <div class="paypal-info">
                    <div class="paypal-icon"><i class="bi bi-paypal"></i></div>
                    <p class="paypal-email">pagamentos@farmavida.com.br</p>
                    <p style="font-size:.8rem;color:var(--texto-sub);margin-top:8px;">
                        Você será redirecionado ao PayPal para finalizar o pagamento com segurança.
                    </p>
                </div>
            </div>

            <!-- Detalhes: Boleto -->
            <div class="pgto-detalhes" id="pgto-detalhe-boleto">
                <div class="boleto-info">
                    <p><i class="bi bi-clock"></i> O boleto será gerado após a confirmação do pedido.</p>
                    <p style="margin-top:8px;"><strong>Prazo de vencimento:</strong> 3 dias úteis.</p>
                    <p style="margin-top:4px;"><strong>Atenção:</strong> O pedido só será processado após a confirmação do pagamento.</p>
                </div>
            </div>

            <div class="feedback-msg" id="fb-pagamento" style="margin-top:12px;"></div>

            <button class="btn-form" id="btn-confirmar-pgto" onclick="confirmarPedido()" disabled
                style="margin-top:16px;background:var(--verde);">
...
<!-- ══ MODAL CONFIRMAÇÃO PIX (pós-pedido) ══ -->
<div class="modal-overlay" id="modal-pix-confirmado">
    <div class="modal-box" style="max-width:420px;">
        <div class="modal-box__header" style="background:#00897B;">
            <h3><i class="bi bi-qr-code-scan"></i> PIX Gerado!</h3>
            <button onclick="fecharModal('pix-confirmado')"><i class="bi bi-x-lg"></i></button>
        </div>
        <div class="modal-box__body">
            <p style="text-align:center;font-size:.9rem;margin-bottom:14px;">
                🎉 Pedido <strong id="pix-pedido-num">#0</strong> criado!<br>
                Pague via PIX para confirmar:
            </p>
            <div class="pix-container">
                <div class="pix-qr-wrap">
                    <canvas id="pix-qr-canvas-final"></canvas>
                </div>
                <div class="pix-copia-cola">
                    <span id="pix-codigo-final" style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></span>
                    <button onclick="copiarPixFinal()"><i class="bi bi-clipboard"></i> Copiar</button>
                </div>
                <ul class="pix-instrucoes" style="list-style:none;padding:0;text-align:left;">
                    <li>📱 Abra seu banco &rarr; Pagar via PIX</li>
                    <li>📷 Escaneie o QR Code ou cole o código</li>
                    <li>✅ Confirme e aguarde a aprovação</li>
                </ul>
            </div>
            <button class="btn-form" onclick="fecharModal('pix-confirmado')" style="margin-top:8px;">
                Fechar
            </button>
        </div>
    </div>
...
// ════════════════════════════════════════════════════
// CHECKOUT — abre modal de pagamento
// ════════════════════════════════════════════════════
let _pgtoSelecionado = null;
...
function checkout() {
    ...
    const total = carrinho.reduce((s,i)=>s+i.quantidade*i.preco_venda,0);
    document.getElementById('pgto-total-display').textContent = 'R$ ' + fmtBRL(total);

    // Reset de seleção
    _pgtoSelecionado = null;
    _pixPayloadAtual = '';
    document.querySelectorAll('.pgto-btn').forEach(b=>b.classList.remove('ativo'));
    document.querySelectorAll('.pgto-detalhes').forEach(d=>d.classList.remove('visivel'));
    document.getElementById('btn-confirmar-pgto').disabled = true;
    document.getElementById('fb-pagamento').textContent = '';
...
function selecionarPgto(tipo) {
    _pgtoSelecionado = tipo;
    document.querySelectorAll('.pgto-btn').forEach(b=>b.classList.remove('ativo'));
    document.querySelectorAll('.pgto-detalhes').forEach(d=>d.classList.remove('visivel'));
    document.querySelector(`.pgto-btn[data-pgto="${tipo}"]`).classList.add('ativo');
    document.getElementById(`pgto-detalhe-${tipo}`).classList.add('visivel');
    document.getElementById('btn-confirmar-pgto').disabled = false;

    // Para PIX: mostra instrução — QR Code real aparece após confirmar o pedido
    if (tipo === 'pix') {
        const canvas = document.getElementById('pix-qr-canvas');
        if (canvas) {
            const ctx = canvas.getContext('2d');
            const size = 180; canvas.width = size; canvas.height = size;
            ctx.fillStyle = '#f0faf4'; ctx.fillRect(0,0,size,size);
            ctx.fillStyle = '#2e7d32'; ctx.font = 'bold 13px sans-serif'; ctx.textAlign = 'center';
            ctx.fillText('Clique em "Confirmar Pedido"', size/2, size/2 - 10);
            ctx.fillText('para gerar o QR Code', size/2, size/2 + 10);
        }
        document.getElementById('pix-codigo-texto').textContent = '(código gerado após confirmar o pedido)';
    }
}

function copiarPix() {
    const txt = _pixPayloadAtual || document.getElementById('pix-codigo-texto').textContent;
    if (!txt || txt.includes('Gerando') || txt.includes('Erro')) {
        toast('Código PIX ainda não disponível', true); return;
    }
    navigator.clipboard.writeText(txt).then(()=>toast('✅ Código PIX copiado!'));
}
function copiarPixFinal() {
    const txt = document.getElementById('pix-codigo-final').textContent;
    navigator.clipboard.writeText(txt).then(()=>toast('Código PIX copiado!'));
}

// Gera QR Code usando API pública de QR (qrserver.com)
function gerarQrCodePix(canvasId, payload) {
    ...
    const url = `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(payload)}&format=png&margin=4&ecc=M`;
    const img = new Image();
    img.crossOrigin = 'anonymous';
    img.onload = () => { ctx.drawImage(img, 0, 0, size, size); };
    img.onerror = () => {
        ctx.fillStyle = '#fff3cd'; ctx.fillRect(0,0,size,size);
        ctx.fillStyle = '#856404'; ctx.font = '11px sans-serif'; ctx.textAlign = 'center';
        ctx.fillText('Use o código abaixo', size/2, size/2-8);
        ctx.fillText('para pagar via PIX', size/2, size/2+10);
    };
    img.src = url;
}

async function confirmarPedido() {
    if (!_pgtoSelecionado) return;
    const fb = document.getElementById('fb-pagamento');
    ...
    fd.append('forma_pagamento', _pgtoSelecionado);
    ...
    try {
        ...
        if (d.success) {
            ...
            if (_pgtoSelecionado === 'pix' && d.pix_payload) {
                // Abre modal de PIX com QR Code real
                document.getElementById('pix-pedido-num').textContent = '#' + d.pedido_id;
                document.getElementById('pix-codigo-final').textContent = d.pix_payload;
                gerarQrCodePix('pix-qr-canvas-final', d.pix_payload);
                abrirModal('pix-confirmado');
            } else {
                toast(`🎉 Pedido #${d.pedido_id} realizado! Forma: ${_pgtoSelecionado.toUpperCase()}`);
            }
        } else {
            fb.textContent = d.message || 'Erro ao criar pedido.';
            fb.className = 'feedback-msg erro';
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-check-circle-fill"></i> Confirmar Pedido';
        }
    } catch(e) {
