...
<script>
    ...
    document.getElementById('formNovoProduto').addEventListener('submit', async function(event) {
        ...
        // Captura dados ANTES de qualquer reset
        const formData = new FormData(form);

        // Desanexa o listener temporariamente para evitar resubmissão
        ...
        try {
            ...
            if (result.success) {
                // Só fecha/reseta após sucesso confirmado pelo servidor
                const modal = bootstrap.Modal.getInstance(document.getElementById('modalNovoProduto'));
                if (modal) {
                    modal.hide();
                    document.getElementById('modalNovoProduto').addEventListener('hidden.bs.modal', () => {
