...
// Garante que coluna 'ativo' existe e UNIQUE constraint está aplicada
...
// Garante UNIQUE para evitar duplicação física no banco
try {
    $hasUniq = $pdo->query("SHOW INDEX FROM produtos WHERE Key_name = 'uq_nome_fabricante'")->fetchAll();
    if (empty($hasUniq)) {
        // Remove duplicados antes de criar constraint
        $pdo->exec("DELETE p1 FROM produtos p1 INNER JOIN produtos p2 ON p1.nome=p2.nome AND p1.fabricante=p2.fabricante AND p1.id>p2.id");
