...
try {
    switch ($endpoint) {
        ...
        case 'produtos_criar':
            ...
            // Verifica se já existe antes de qualquer coisa
            $chk = $pdo->prepare("SELECT id FROM produtos WHERE nome = ? AND fabricante = ? LIMIT 1");
            ...
            // INSERT IGNORE — banco rejeita silenciosamente se vier duplicado por race condition
            $stmt = $pdo->prepare("INSERT IGNORE INTO produtos (nome, fabricante, categoria, preco_venda, descricao, foto, ativo) VALUES (?, ?, ?, ?, ?, ?, 1)");
            $stmt->execute([$nome, $fabric, $cat, $preco, $desc, $foto]);
            $produtoId = (int)$pdo->lastInsertId();

            // Se lastInsertId for 0, o INSERT IGNORE ignorou (duplicado) — busca o existente
            if (!$produtoId) {
                $chk2 = $pdo->prepare("SELECT id FROM produtos WHERE nome = ? AND fabricante = ? LIMIT 1");
                $chk2->execute([$nome, $fabric]);
                $produtoId = (int)$chk2->fetchColumn();
            }
