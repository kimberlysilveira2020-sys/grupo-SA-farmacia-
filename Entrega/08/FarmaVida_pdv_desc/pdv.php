            ...
            <?php if (!empty($produtos)): ?>
                ...
                <div class="produto-card <?= $tem_desconto ? 'produto-card--promo' : '' ?> <?= $sem_estoque ? 'produto-card--esgotado' : '' ?>"
                    ...
                    <div class="produto-card__info">
                        ...
                        <p class="produto-card__desc"><?= htmlspecialchars($produto['descricao']) ?></p>
                        <?php endif; ?>
...
.produto-card:hover { box-shadow: 0 4px 14px rgba(25,118,210,.18); transform: translateY(-2px); }
.produto-card--promo { border-color: #e53935; border-top: 3px solid #e53935; }
...
.produto-card__nome { font-size: .88rem; font-weight: 700; color: #212121; margin: 0 0 1px; line-height: 1.2; }
.produto-card__fab  { font-size: .72rem; color: #9e9e9e; margin: 0 0 4px; }
.produto-card__controlado { font-size: .65rem; margin-bottom: 4px; }

/* Descrição — aparece em hover */
.produto-card__desc {
    font-size: .72rem;
    color: #616161;
    margin: 0 0 6px;
...
.produto-card:hover .produto-card__desc {
    max-height: 40px;
    opacity: 1;
    margin-bottom: 6px;
}

.produto-card__preco-wrap { display: flex; align-items: baseline; gap: 5px; margin-bottom: 3px; }
.produto-card__preco { font-size: .95rem; font-weight: 700; color: #2e7d32; }
.produto-card__preco--promo { color: #e53935; }
.produto-card__preco-orig { font-size: .72rem; color: #9e9e9e; text-decoration: line-through; }

.produto-card__estoque { font-size: .7rem; margin-bottom: 8px; }
