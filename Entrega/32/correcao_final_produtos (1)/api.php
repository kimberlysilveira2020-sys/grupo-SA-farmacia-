...
try {
    switch ($endpoint) {
        ...
        case 'produtos_criar':
            ...
            // Verifica se já existe ATIVO (ignora soft-deleted)
            ...
            // INSERT direto — a verificação acima já garantiu que não existe ativo com esse nome+fabricante
            ...
            // Fallback de segurança (nunca deve cair aqui em condições normais)
            if (!$produtoId) {
                $chk2 = $pdo->prepare("SELECT id FROM produtos WHERE nome = ? AND fabricante = ? AND COALESCE(ativo,1) = 1 LIMIT 1");
