            ...
            <div id="carouselBanners" class="carousel slide carousel-fade" data-bs-ride="carousel" data-bs-interval="5000">
                ...
                <div class="carousel-inner" style="border-radius: 0 0 10px 10px; overflow:hidden;">
                    ...
                    <div class="carousel-item <?= $i === 0 ? 'active' : '' ?>">
                        <?php if (!empty($banner['imagem']) && file_exists('<?= BASE_URL ?>uploads/banners/' . $banner['imagem'])): ?>
                            <img src="<?= BASE_URL ?>uploads/banners/<?= htmlspecialchars($banner['imagem']) ?>" class="d-block w-100" style="max-height:260px; object-fit:cover;" alt="<?= htmlspecialchars($banner['titulo']) ?>">
...
<script>
    ...
    async function salvarBanner() {
        ...
        try {
            const res = await fetch('<?= BASE_URL ?>dashboard/bannerCriar', { method: 'POST', body: fd });
    ...
    async function deletarBanner(id) {
        ...
        const res = await fetch('<?= BASE_URL ?>dashboard/bannerDeletar', {
            method: 'POST',
