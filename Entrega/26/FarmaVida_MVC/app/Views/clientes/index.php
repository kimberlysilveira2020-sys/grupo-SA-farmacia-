    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm text-center py-3">
                ...
                <h4 class="fw-bold mb-0 mt-1"><?= $total_loja ?></h4>
                <small class="text-muted">Cadastrados na Loja</small>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm text-center py-3">
                <i class="bi bi-person-badge text-warning" style="font-size:2rem;"></i>
                <h4 class="fw-bold mb-0 mt-1"><?= $total_interno ?></h4>
                <small class="text-muted">Cadastros Internos</small>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm text-center py-3">
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <div class="input-group">
                ...
                <input type="text" class="form-control" id="campo-busca"
                       placeholder="Buscar por nome, CPF, telefone ou e-mail..."
                       value="<?= htmlspecialchars($busca) ?>"
                       onkeydown="if(event.key==='Enter') buscarClientes()">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-3">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold text-white"
                                         style="width:36px;height:36px;background:<?= $c['origem']==='loja' ? '#388E3C' : '#1976D2' ?>;font-size:.85rem;flex-shrink:0;">
                            ...
                            <td>
                                ...
                                <?php if ($c['email']): ?>
                                    <br><small class="text-muted"><i class="bi bi-envelope"></i> <?= htmlspecialchars($c['email']) ?></small>
                                <?php endif; ?>
                            </td>
                            <td class="text-center">
                                <?php if ($c['origem'] === 'loja'): ?>
                                    <span class="badge bg-success"><i class="bi bi-shop"></i> Loja Online</span>
                                <?php else: ?>
                                    <span class="badge bg-primary"><i class="bi bi-building"></i> Interno</span>
                                <?php endif; ?>
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center pe-3">
                                ...
                                <?php else: ?>
                                <span class="text-muted small" title="Gerenciado pelo próprio cliente na loja">
                                    <i class="bi bi-lock"></i> Loja
                                </span>
                                <?php endif; ?>
                            </td>
                        </tr>
...
<script>
    ...
    async function salvarCliente() {
        ...
        const res  = await fetch('<?= BASE_URL ?>clientes/criar', { method:'POST', body:fd });
    ...
    async function atualizarCliente() {
        ...
        const res  = await fetch('<?= BASE_URL ?>clientes/editar', { method:'POST', body:fd });
        const data = await res.json();
        if (data.success) location.reload();
        else fb.innerHTML = `<div class="alert alert-danger py-2">${data.message}</div>`;
    }
    ...
    async function deletarCliente(id, nome) {
        ...
        const res  = await fetch('<?= BASE_URL ?>clientes/deletar', {
