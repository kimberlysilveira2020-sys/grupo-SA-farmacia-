                ...
                <button class="accordion-button collapsed" type="button"
                    ...
                    <span class="d-flex justify-content-between w-100 me-3 align-items-center">
                        <span class="d-flex align-items-center gap-2">
                            <?php if (!empty($produto['foto']) && file_exists('<?= BASE_URL ?>uploads/produtos/' . $produto['foto'])): ?>
                            <img src="<?= BASE_URL ?>uploads/produtos/<?= htmlspecialchars($produto['foto']) ?>" 
...
<script>
    ...
    document.getElementById('formNovoProduto').addEventListener('submit', async function(event) {
        ...
        try {
            ...
            const res    = await fetch('<?= BASE_URL ?>produtos/criar', { method: 'POST', body: formData });
    ...
    document.getElementById('formNovoLote').addEventListener('submit', async function(event) {
        ...
        btn.disabled = true;
        try {
            const res    = await fetch('<?= BASE_URL ?>produtos/loteCriar', { method: 'POST', body: new FormData(this) });
            const result = await res.json();
    ...
    document.getElementById('formEditarProduto').addEventListener('submit', async function(event) {
        ...
        btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Salvando...';
        try {
            const res    = await fetch('<?= BASE_URL ?>produtos/editar', { method: 'POST', body: new FormData(this) });
            const result = await res.json();
            if (result.success) location.reload();
            else {
                alert('Erro: ' + result.message);
    ...
    async function deletarProduto(id) {
        ...
        try {
            const res    = await fetch('<?= BASE_URL ?>produtos/deletar', {
    ...
    async function carregarLotes(produtoId) {
        ...
        try {
            const res  = await fetch(`<?= BASE_URL ?>produtos/lotes?produto_id=${produtoId}`);
