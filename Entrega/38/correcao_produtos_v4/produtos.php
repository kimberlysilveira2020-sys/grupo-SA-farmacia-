...
unset($produto); // OBRIGATÓRIO: quebra a referência ao último elemento antes do próximo foreach
...
<script>
    // ── Modal de notificação / confirmação ───────────────────────────────
    // Substitui alert() e confirm() por modais Bootstrap.

    /**
     * Exibe um modal de aviso/erro/sucesso.
     * @param {string} mensagem  Texto exibido no corpo do modal.
     * @param {string} tipo      'erro' | 'aviso' | 'sucesso' (controla cor e ícone)
     */
    function mostrarModal(mensagem, tipo = 'erro') {
        const config = {
            erro:    { bg: 'bg-danger',  icone: 'bi-x-circle-fill',        titulo: 'Erro' },
            aviso:   { bg: 'bg-warning text-dark', icone: 'bi-exclamation-triangle-fill', titulo: 'Atenção' },
            sucesso: { bg: 'bg-success', icone: 'bi-check-circle-fill',     titulo: 'Sucesso' },
        };
        const c = config[tipo] || config.erro;
        document.getElementById('notif-header').className  = 'modal-header ' + c.bg + (tipo === 'aviso' ? '' : ' text-white');
        document.getElementById('notif-icone').className   = 'bi ' + c.icone + ' me-2';
        document.getElementById('notif-titulo').textContent = c.titulo;
        document.getElementById('notif-corpo').textContent  = mensagem;
        new bootstrap.Modal(document.getElementById('modalNotificacao')).show();
    }

    /**
     * Exibe um modal de confirmação e resolve com true/false.
     * @param {string} mensagem  Pergunta exibida.
     * @returns {Promise<boolean>}
     */
    function confirmarModal(mensagem) {
        return new Promise(resolve => {
            document.getElementById('confirm-corpo').textContent = mensagem;
            const modalEl = document.getElementById('modalConfirmacao');
            const modal   = new bootstrap.Modal(modalEl);
            const btnSim  = document.getElementById('confirm-sim');

            const onSim = () => { modal.hide(); resolve(true); };
    ...
    function previewFoto(input, imgId, placeholderId) {
        ...
        if (file.size > 2 * 1024 * 1024) {
            mostrarModal('Imagem muito grande. O tamanho máximo é 2MB.', 'aviso');
            input.value = '';
            return;
        }
    ...
    document.getElementById('formNovoProduto').addEventListener('submit', async function(event) {
        ...
        try {
            const res    = await fetch('api.php?endpoint=produtos_criar', { method: 'POST', body: new FormData(form) });
            ...
            if (result.success) {
                ...
                mostrarModal(result.message, 'erro');
                btn.disabled = false;
                btn.innerHTML = '<i class="bi bi-save"></i> Salvar Produto';
                _salvandoProduto = false;
            }
        } catch (e) {
            mostrarModal('Não foi possível salvar o produto. Tente novamente.', 'erro');
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-save"></i> Salvar Produto';
            _salvandoProduto = false;
        }
    ...
    document.getElementById('formNovoLote').addEventListener('submit', async function(event) {
        ...
        try {
            ...
            if (result.success) {
                ...
                mostrarModal(result.message, 'erro');
                btn.disabled = false;
                btn.innerHTML = '<i class="bi bi-save"></i> Adicionar Lote';
            }
        } catch (e) {
            mostrarModal('Não foi possível salvar o lote. Tente novamente.', 'erro');
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-save"></i> Adicionar Lote';
        }
    });
    ...
    document.getElementById('formEditarProduto').addEventListener('submit', async function(event) {
        ...
        try {
            ...
            mostrarModal('Não foi possível atualizar o produto. Tente novamente.', 'erro');
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-save"></i> Salvar Alterações';
        }
    });
    ...
    async function deletarProduto(id) {
        const confirmado = await confirmarModal('Excluir este produto e todos os seus lotes? Esta ação não pode ser desfeita.');
        ...
        try {
            ...
            if (result.success) {
                if (result.aviso) {
                    mostrarModal(result.aviso, 'aviso');
                    document.getElementById('modalNotificacao').addEventListener('hidden.bs.modal', () => location.reload(), { once: true });
                } else {
                    location.reload();
                }
            } else {
            ...
            mostrarModal('Não foi possível excluir o produto. Tente novamente.', 'erro');
        }
    }
...
<!-- ===== MODAL NOTIFICAÇÃO (erro / aviso / sucesso) ===== -->
<div class="modal fade" id="modalNotificacao" tabindex="-1" aria-modal="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow">
            <div class="modal-header" id="notif-header">
                <h5 class="modal-title d-flex align-items-center">
                    <i id="notif-icone" class="bi me-2"></i>
                    <span id="notif-titulo"></span>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body" id="notif-corpo"></div>
...
<div class="modal fade" id="modalConfirmacao" tabindex="-1" aria-modal="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content shadow">
            ...
            <div class="modal-body" id="confirm-corpo"></div>
            <div class="modal-footer border-0 pt-0">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-danger" id="confirm-sim">
