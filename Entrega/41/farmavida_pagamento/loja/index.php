...
══════════════════════════════════════════════════════════ */
#modal-checkout .modal-box { max-width: 520px; }
...
.step__dot {
    ...
    border-radius: 50%;
    border: 2.5px solid var(--cinza-bord);
    background: #fff;
    display: flex; align-items: center; justify-content: center;
    font-size: .8rem;
    font-weight: 800;
...
.step__dot.ativo { background: var(--verde); border-color: var(--verde); color: #fff; }
.step__dot.done  { background: var(--verde-md); border-color: var(--verde-md); color: #fff; }
.step__label { font-size: .68rem; font-weight: 700; color: var(--texto-sub); text-align: center; }
.step__label.ativo { color: var(--verde); }
...
/* Painéis de step */
.checkout-panel { display: none; padding: 16px 24px 20px; }
.checkout-panel.ativo { display: block; }
...
.resumo-item__nome { flex: 1; font-weight: 600; }
.resumo-item__qtd  { color: var(--texto-sub); font-size: .78rem; min-width: 40px; text-align: center; }
.resumo-item__val  { font-weight: 800; color: var(--verde); min-width: 72px; text-align: right; }
...
.resumo-total span:last-child { font-size: 1.25rem; }
...
.metodo-btn {
    border: 2px solid var(--cinza-bord);
    background: #fff;
    border-radius: 12px;
    padding: 16px 12px;
    display: flex;
...
.metodo-btn i { font-size: 1.8rem; }
.metodo-btn:hover { border-color: var(--verde); color: var(--verde); background: var(--verde-clr); }
...
.card-flags { display: flex; gap: 6px; margin-top: 6px; }
.card-flags span {
    font-size: .7rem; font-weight: 700; padding: 2px 7px;
    border-radius: 4px; background: var(--cinza-bord);
    color: var(--texto-sub);
}
...
.pix-chave {
    background: var(--cinza-bg);
    border: 1.5px dashed var(--cinza-bord);
    border-radius: 8px;
    padding: 10px 14px;
    font-size: .82rem;
...
.pix-chave:hover { background: #eef2f5; }
.pix-instrucoes { font-size: .78rem; color: var(--texto-sub); margin-top: 10px; line-height: 1.5; }

/* Dinheiro */
.troco-box { margin-top: 6px; }
...
.btn-voltar-step:hover { border-color: var(--texto); color: var(--texto); }
.btn-avancar-step {
    flex: 1;
    background: var(--verde);
    color: #fff;
...
.btn-avancar-step:hover { background: var(--verde-esc); }
.btn-avancar-step:disabled { background: #bdbdbd; cursor: not-allowed; }
.btn-avancar-step.pagar { background: var(--laranja); }
.btn-avancar-step.pagar:hover { background: #d96f0a; }
...
.sucesso-titulo { font-size: 1.4rem; font-weight: 900; color: var(--texto); margin-bottom: 6px; }
.sucesso-sub    { font-size: .88rem; color: var(--texto-sub); margin-bottom: 20px; }
.sucesso-pedido {
    background: var(--verde-clr);
    border-radius: 12px;
    padding: 16px;
...
.sucesso-pedido small { font-size: .72rem; color: var(--texto-sub); font-weight: 700; text-transform: uppercase; }
.sucesso-pedido p { margin: 3px 0 0; font-weight: 800; color: var(--verde-esc); font-size: 1rem; }
.btn-fechar-sucesso {
    width: 100%;
    background: var(--verde);
    color: #fff;
...
<div class="cart-drawer" id="cart-drawer">
    ...
    <div class="cart-drawer__footer">
        ...
        <button class="btn-checkout" id="btn-checkout" onclick="abrirCheckout()" disabled>
            <i class="bi bi-credit-card-fill"></i> Ir para Pagamento
        </button>
    </div>
</div>
...
<div class="modal-overlay" id="modal-checkout">
    <div class="modal-box" id="checkout-box">
        ...
        <!-- Header dinâmico -->
        <div class="modal-box__header" id="checkout-header">
            <h3 id="checkout-titulo"><i class="bi bi-bag-check"></i> Finalizar Pedido</h3>
            <button onclick="fecharModal('checkout')"><i class="bi bi-x-lg"></i></button>
        </div>

        <!-- Stepper (oculto na tela de sucesso) -->
        <div id="checkout-stepper" class="checkout-steps">
            <div class="step" id="step-ind-1">
                <div class="step__dot ativo" id="step-dot-1">1</div>
                <span class="step__label ativo" id="step-lbl-1">Resumo</span>
            </div>
            <div class="step-line" id="step-line-1"></div>
            <div class="step" id="step-ind-2">
                <div class="step__dot" id="step-dot-2">2</div>
                <span class="step__label" id="step-lbl-2">Pagamento</span>
            </div>
            <div class="step-line" id="step-line-2"></div>
            <div class="step" id="step-ind-3">
                <div class="step__dot" id="step-dot-3">3</div>
                <span class="step__label" id="step-lbl-3">Confirmar</span>
            </div>
        </div>

        <!-- ─── STEP 1: Resumo do pedido ─── -->
        <div class="checkout-panel ativo" id="checkout-step-1">
            <div id="checkout-resumo-itens"></div>
            <div class="resumo-total">
                <span>Total</span>
                <span id="checkout-total-val">R$ 0,00</span>
            </div>
            <div class="form-group" style="margin-top:14px;">
                <label>Observação (opcional)</label>
                <input type="text" id="checkout-obs" placeholder="Ex: Por favor, embrulhe para presente...">
            </div>
        </div>

        <!-- ─── STEP 2: Método de pagamento ─── -->
        <div class="checkout-panel" id="checkout-step-2">
            <p style="font-size:.82rem;color:var(--texto-sub);margin-bottom:14px;">Escolha como deseja pagar:</p>

            <div class="metodos-grid">
                <button class="metodo-btn" id="metodo-cartao-cred" onclick="selecionarMetodo('cartao_credito')">
                    <i class="bi bi-credit-card"></i>
                    Cartão de Crédito
                </button>
                <button class="metodo-btn" id="metodo-cartao-deb" onclick="selecionarMetodo('cartao_debito')">
                    <i class="bi bi-credit-card-2-back"></i>
                    Cartão de Débito
                </button>
                <button class="metodo-btn" id="metodo-pix" onclick="selecionarMetodo('pix')">
                    <i class="bi bi-qr-code"></i>
                    Pix
                </button>
                <button class="metodo-btn" id="metodo-dinheiro" onclick="selecionarMetodo('dinheiro')">
            ...
            <div id="form-cartao-credito" style="display:none;">
                <div class="form-group">
                    <label>Número do Cartão</label>
                    <div class="card-input-wrap">
                        <input type="text" id="cc-numero" placeholder="0000 0000 0000 0000"
                               maxlength="19" oninput="mascaraCartao(this)" autocomplete="cc-number">
                        <i class="bi bi-credit-card" id="cc-bandeira-icon"></i>
                    </div>
                    <div class="card-flags">
                        <span>Visa</span><span>Master</span><span>Elo</span><span>Amex</span><span>Hiper</span>
                    </div>
                </div>
                <div class="form-group">
                    <label>Nome no Cartão</label>
                    <input type="text" id="cc-nome" placeholder="Como impresso no cartão"
                           style="text-transform:uppercase;" oninput="this.value=this.value.toUpperCase()" autocomplete="cc-name">
                </div>
                <div class="row-2">
                    <div class="form-group">
                        <label>Validade</label>
                        <input type="text" id="cc-validade" placeholder="MM/AA" maxlength="5"
                ...
                <div class="form-group parcelas-select">
                ...
                <div class="feedback-msg" id="fb-cartao-cred"></div>
            </div>

            <!-- Formulário Cartão de Débito -->
            <div id="form-cartao-debito" style="display:none;">
                <div class="form-group">
                    <label>Número do Cartão</label>
                    <div class="card-input-wrap">
                        <input type="text" id="cd-numero" placeholder="0000 0000 0000 0000"
                               maxlength="19" oninput="mascaraCartao(this)" autocomplete="cc-number">
                        <i class="bi bi-credit-card-2-back"></i>
                    </div>
                    <div class="card-flags"><span>Visa</span><span>Master</span><span>Elo</span></div>
                </div>
                <div class="form-group">
                    <label>Nome no Cartão</label>
                    <input type="text" id="cd-nome" placeholder="Como impresso no cartão"
                           style="text-transform:uppercase;" oninput="this.value=this.value.toUpperCase()" autocomplete="cc-name">
                </div>
                <div class="row-2">
                    <div class="form-group">
                        <label>Validade</label>
                        <input type="text" id="cd-validade" placeholder="MM/AA" maxlength="5"
                               oninput="mascaraValidade(this)" autocomplete="cc-exp">
                    </div>
                    <div class="form-group">
                        <label>CVV</label>
                        <input type="text" id="cd-cvv" placeholder="•••" maxlength="4"
                               oninput="this.value=this.value.replace(/\D/g,'')">
                    </div>
                </div>
                <div class="feedback-msg" id="fb-cartao-deb"></div>
            </div>

            <!-- Pix -->
            <div id="form-pix" style="display:none;">
                <div class="pix-box">
                    <div class="pix-qr"><i class="bi bi-qr-code-scan"></i></div>
                    <p style="font-size:.82rem;font-weight:700;margin-bottom:8px;">Ou copie a chave Pix:</p>
                    <div class="pix-chave" onclick="copiarPix(this)" title="Clique para copiar">
                        farmavida@pix.com.br
                        <i class="bi bi-clipboard" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);color:var(--texto-sub);"></i>
                    </div>
                    <p class="pix-instrucoes">
                        Após o pagamento, seu pedido será <strong>confirmado automaticamente</strong>.<br>
                        Prazo de compensação: até 10 minutos.
                    </p>
                </div>
            </div>

            <!-- Dinheiro -->
            <div id="form-dinheiro" style="display:none;">
                <div class="form-group">
                    <label><i class="bi bi-info-circle"></i> Retire e pague na farmácia</label>
                    <p style="font-size:.82rem;color:var(--texto-sub);margin:4px 0 12px;">
                        Seu pedido será separado e aguardará a sua retirada. Pague na loja no momento da retirada.
                    </p>
                </div>
                <div class="form-group troco-box">
                    <label>Precisa de troco para quanto?</label>
                    <input type="number" id="dinheiro-troco" placeholder="Ex: 50,00" step="0.01" min="0">
                    <small style="color:var(--texto-sub);font-size:.75rem;">Deixe em branco se não precisar de troco</small>
                </div>
            </div>
        </div>

        <!-- ─── STEP 3: Confirmação ─── -->
        <div class="checkout-panel" id="checkout-step-3">
            <div id="confirmacao-body">
                <!-- Preenchido via JS -->
            </div>
            <div class="feedback-msg" id="fb-checkout"></div>
        </div>

        <!-- ─── SUCESSO ─── -->
        <div class="checkout-panel" id="checkout-sucesso" style="display:none;">
            <div class="checkout-sucesso">
                <div class="sucesso-icon"><i class="bi bi-check2"></i></div>
                <p class="sucesso-titulo">Pedido Realizado! 🎉</p>
                <p class="sucesso-sub">Obrigado pela sua compra. Seu pedido já está sendo processado.</p>
                <div class="sucesso-pedido" id="sucesso-detalhes">
                    <small>Número do Pedido</small>
                    <p id="sucesso-pedido-num">—</p>
                </div>
                <button class="btn-fechar-sucesso" onclick="finalizarSucesso()">
                    <i class="bi bi-house-fill"></i> Voltar à Loja
                </button>
            </div>
        </div>

        <!-- Navegação entre steps -->
        <div class="checkout-nav" id="checkout-nav">
            <button class="btn-voltar-step" id="btn-voltar" onclick="voltarStep()" style="display:none;">
                <i class="bi bi-chevron-left"></i> Voltar
            </button>
            <button class="btn-avancar-step" id="btn-avancar" onclick="avancarStep()">
...
function abrirCheckout() {
    ...
    document.getElementById('fb-checkout').className = 'feedback-msg';

    // Limpa seleção de método
    document.querySelectorAll('.metodo-btn').forEach(b => b.classList.remove('selecionado'));
    ['form-cartao-credito','form-cartao-debito','form-pix','form-dinheiro'].forEach(id => {
...
function renderResumoCheckout() {
    const total = carrinho.reduce((s,i) => s + i.preco_venda * i.quantidade, 0);
    document.getElementById('checkout-total-val').textContent = 'R$ ' + fmtBRL(total);

    document.getElementById('checkout-resumo-itens').innerHTML = carrinho.map(item => `
        <div class="resumo-item">
            <span class="resumo-item__nome">${item.nome}</span>
            <span class="resumo-item__qtd">× ${item.quantidade}</span>
            <span class="resumo-item__val">R$ ${fmtBRL(item.preco_venda * item.quantidade)}</span>
        </div>`).join('');
}
...
function renderConfirmacao() {
    const total     = carrinho.reduce((s,i) => s + i.preco_venda * i.quantidade, 0);
    const metodosLabel = {
        cartao_credito: `Cartão de Crédito (${document.getElementById('cc-parcelas')?.value || 1}x)`,
    ...
    if (metodoPagamento === 'cartao_credito') {
        const num = document.getElementById('cc-numero').value;
        extraInfo = `<p style="font-size:.82rem;color:var(--texto-sub);margin-top:4px;">Cartão ****&nbsp;${num.slice(-4)}</p>`;
    }
    if (metodoPagamento === 'dinheiro') {
        const troco = document.getElementById('dinheiro-troco').value;
        if (troco) extraInfo = `<p style="font-size:.82rem;color:var(--texto-sub);margin-top:4px;">Troco para R$ ${fmtBRL(troco)}</p>`;
    }

    document.getElementById('confirmacao-body').innerHTML = `
        <div style="margin-bottom:14px;">
            <p style="font-size:.72rem;font-weight:700;color:var(--texto-sub);text-transform:uppercase;margin-bottom:8px;">Itens do Pedido</p>
            ${carrinho.map(i => `
            <div style="display:flex;justify-content:space-between;font-size:.82rem;padding:4px 0;gap:8px;">
                <span>${i.nome} <span style="color:var(--texto-sub);">× ${i.quantidade}</span></span>
                <strong>R$ ${fmtBRL(i.preco_venda * i.quantidade)}</strong>
            </div>`).join('')}
        </div>
        ${obs ? `<div style="background:var(--cinza-bg);border-radius:8px;padding:10px 12px;font-size:.82rem;margin-bottom:12px;">
            <i class="bi bi-chat-left-text"></i> ${obs}
        </div>` : ''}
        <div style="background:var(--verde-clr);border-radius:10px;padding:14px 16px;display:flex;flex-direction:column;gap:4px;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <span style="font-size:.82rem;color:var(--texto-sub);">Forma de Pagamento</span>
                <strong style="color:var(--verde-esc);">${metodosLabel[metodoPagamento] || ''}</strong>
            </div>
            ${extraInfo}
            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px;border-top:1px solid #c8e6da;padding-top:8px;">
                <span style="font-weight:800;color:var(--texto);">Total a Pagar</span>
                <span style="font-size:1.25rem;font-weight:900;color:var(--verde);">R$ ${fmtBRL(total)}</span>
            </div>
        </div>`;
}
...
function selecionarMetodo(metodo) {
    ...
    document.querySelectorAll('.metodo-btn').forEach(b => b.classList.remove('selecionado'));
    document.getElementById('metodo-' + metodo.replace('_','-')).classList.add('selecionado');
    ...
    const formMap = {
        cartao_credito: 'form-cartao-credito',
        cartao_debito:  'form-cartao-debito',
        pix:            'form-pix',
        dinheiro:       'form-dinheiro'
    };
    if (formMap[metodo]) {
        document.getElementById(formMap[metodo]).style.display = 'block';
    }

    // Atualizar parcelas com base no total
    if (metodo === 'cartao_credito') atualizarParcelas();
...
function atualizarParcelas() {
    const total = carrinho.reduce((s,i) => s + i.preco_venda * i.quantidade, 0);
    const sel   = document.getElementById('cc-parcelas');
    if (!sel) return;
    sel.innerHTML = '';
    const max = Math.min(12, Math.floor(total / 10)) || 1;
    for (let n = 1; n <= max; n++) {
        const val = (total / n).toFixed(2).replace('.',',');
        sel.innerHTML += `<option value="${n}">${n}x de R$ ${val}${n <= 6 ? ' sem juros' : ''}</option>`;
    }
}
...
function validarStep() {
    if (checkoutStep === 2) {
        if (!metodoPagamento) {
            toast('Selecione uma forma de pagamento', true); return false;
        }
        if (metodoPagamento === 'cartao_credito') {
            const num  = document.getElementById('cc-numero').value.replace(/\D/g,'');
            const nome = document.getElementById('cc-nome').value.trim();
            const val  = document.getElementById('cc-validade').value;
            const cvv  = document.getElementById('cc-cvv').value;
            if (num.length < 13 || !nome || val.length < 5 || cvv.length < 3) {
                const fb = document.getElementById('fb-cartao-cred');
                fb.textContent = 'Preencha todos os dados do cartão corretamente.';
                fb.className   = 'feedback-msg erro';
                return false;
            }
            document.getElementById('fb-cartao-cred').className = 'feedback-msg';
        }
        if (metodoPagamento === 'cartao_debito') {
            const num  = document.getElementById('cd-numero').value.replace(/\D/g,'');
            const nome = document.getElementById('cd-nome').value.trim();
            const val  = document.getElementById('cd-validade').value;
            const cvv  = document.getElementById('cd-cvv').value;
            if (num.length < 13 || !nome || val.length < 5 || cvv.length < 3) {
                const fb = document.getElementById('fb-cartao-deb');
                fb.textContent = 'Preencha todos os dados do cartão corretamente.';
                fb.className   = 'feedback-msg erro';
                return false;
            }
            document.getElementById('fb-cartao-deb').className = 'feedback-msg';
        }
    }
    return true;
}
...
function voltarStep() {
    if (checkoutStep > 1) irStep(checkoutStep - 1);
}
...
function irStep(n) {
    ...
    // Painéis
    for (let i = 1; i <= 3; i++) {
        document.getElementById(`checkout-step-${i}`).classList.toggle('ativo', i === n);
    }
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        dot.className = 'step__dot' + (i < n ? ' done' : i === n ? ' ativo' : '');
        lbl.className = 'step__label' + (i === n ? ' ativo' : '');
    ...
    // Botões de navegação
    document.getElementById('btn-voltar').style.display = n > 1 ? 'block' : 'none';

    const btnAv = document.getElementById('btn-avancar');
    if (n === 1) {
        btnAv.innerHTML = 'Continuar <i class="bi bi-chevron-right"></i>';
        btnAv.className = 'btn-avancar-step';
        btnAv.disabled  = false;
    } else if (n === 2) {
        btnAv.innerHTML = 'Revisar Pedido <i class="bi bi-chevron-right"></i>';
        btnAv.className = 'btn-avancar-step';
        btnAv.disabled  = !metodoPagamento;
    } else if (n === 3) {
        btnAv.innerHTML = '<i class="bi bi-lock-fill"></i> Confirmar e Pagar';
        btnAv.className = 'btn-avancar-step pagar';
        btnAv.disabled  = false;
    }

    // Títulos
    const titulos = ['Resumo do Pedido', 'Forma de Pagamento', 'Confirmar Pedido'];
    document.getElementById('checkout-titulo').innerHTML =
        `<i class="bi bi-bag-check"></i> ${titulos[n - 1]}`;
}

async function enviarPedido() {
    const btn = document.getElementById('btn-avancar');
    const fb  = document.getElementById('fb-checkout');
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" style="width:16px;height:16px;border-width:2px;"></span> Processando...';

    const itens  = carrinho.map(i => ({ produto_id: i.id, quantidade: i.quantidade, preco: i.preco_venda }));
    const obs    = document.getElementById('checkout-obs').value.trim();
    const troco  = metodoPagamento === 'dinheiro' ? (document.getElementById('dinheiro-troco').value || null) : null;
    const parcelas = metodoPagamento === 'cartao_credito' ? (document.getElementById('cc-parcelas')?.value || 1) : 1;

    const fd = new FormData();
    fd.append('itens',            JSON.stringify(itens));
    fd.append('metodo_pagamento', metodoPagamento);
    fd.append('observacao',       obs);
    fd.append('parcelas',         parcelas);
    if (troco) fd.append('troco_para', troco);
    ...
    try {
        ...
        if (d.success) {
            // Oculta stepper e painéis
            document.getElementById('checkout-stepper').style.display = 'none';
            document.getElementById('checkout-nav').style.display     = 'none';
            for (let i = 1; i <= 3; i++)
                document.getElementById(`checkout-step-${i}`).classList.remove('ativo');

            document.getElementById('checkout-sucesso').style.display = 'block';
            document.getElementById('sucesso-pedido-num').textContent = `#${d.pedido_id}`;
            ...
            document.getElementById('sucesso-detalhes').innerHTML = `
                <div style="display:flex;gap:16px;flex-wrap:wrap;">
                    <div><small>Número do Pedido</small><p>#${d.pedido_id}</p></div>
                    <div><small>Pagamento</small><p>${metLabel[metodoPagamento] || metodoPagamento}</p></div>
                    <div><small>Status</small><p style="color:var(--verde)">Pendente ✓</p></div>
                </div>`;

            // Limpa carrinho
            carrinho = []; salvarCarrinho(); atualizarCarrinho();
        } else {
            fb.textContent = d.message || 'Erro ao finalizar pedido.';
            fb.className   = 'feedback-msg erro';
            btn.disabled   = false;
            btn.innerHTML  = '<i class="bi bi-lock-fill"></i> Confirmar e Pagar';
        }
    } catch (e) {
...
function finalizarSucesso() {
    ...
    // Restaura estrutura do modal para próximo uso
    setTimeout(() => {
        document.getElementById('checkout-stepper').style.display = '';
        document.getElementById('checkout-nav').style.display     = '';
        document.getElementById('checkout-sucesso').style.display = 'none';
    }, 300);
}

function copiarPix(el) {
    const chave = el.textContent.trim().replace(/\s+/g, '');
...
async function carregarPedidos() {
    ...
    const statusCor   = {pendente:'#f5820d', confirmado:'#2e7d32', cancelado:'#c62828', pronto:'#1565C0', entregue:'#555'};
    const metodosLabel = {
    ...
    body.innerHTML = d.pedidos.map(p=>`
        <div style="padding:12px 0;border-bottom:1px solid #eee;">
            ...
            ${p.metodo_pagamento ? `<div style="font-size:.75rem;color:var(--texto-sub);margin-top:3px;">${metodosLabel[p.metodo_pagamento]||p.metodo_pagamento}${p.metodo_pagamento==='cartao_credito'&&p.parcelas>1?' — '+p.parcelas+'x':''}</div>` : ''}
            ${p.observacao ? `<div style="font-size:.75rem;color:var(--texto-sub);margin-top:2px;font-style:italic;">💬 ${p.observacao}</div>` : ''}
...
function mascaraCartao(input) {
    ...
    input.value = v;
    // Detecta bandeira para ícone
    const num = v.replace(/\D/g,'');
    const icon = document.getElementById('cc-bandeira-icon');
    if (!icon) return;
    if (/^4/.test(num))       icon.className = 'bi bi-credit-card';      // Visa
    else if (/^5[1-5]/.test(num) || /^2[2-7]/.test(num)) icon.className = 'bi bi-credit-card-fill'; // Master
