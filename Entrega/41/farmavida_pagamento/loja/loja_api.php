...
try {
    switch ($endpoint) {
        ...
        case 'pedido_criar':
            ...
            $troco    = !empty($_POST['troco_para']) ? round((float)$_POST['troco_para'], 2) : null;

            $metodosPermitidos = ['cartao_credito','cartao_debito','pix','dinheiro'];
            if (!in_array($metodo, $metodosPermitidos)) {
                jsonResp(['success'=>false,'message'=>'Método de pagamento inválido.']);
            }

            // Garante colunas extras na tabela pedidos (idempotente)
            try {
                $cols = $pdo->query("SHOW COLUMNS FROM pedidos")->fetchAll(PDO::FETCH_COLUMN);
                if (!in_array('metodo_pagamento', $cols)) {
                    $pdo->exec("ALTER TABLE pedidos
                        ADD COLUMN `metodo_pagamento` VARCHAR(30)  DEFAULT NULL AFTER `total`,
                        ADD COLUMN `parcelas`         TINYINT(2)  DEFAULT 1    AFTER `metodo_pagamento`,
                        ADD COLUMN `troco_para`       DECIMAL(10,2) DEFAULT NULL AFTER `parcelas`,
                        ADD COLUMN `observacao`       TEXT         DEFAULT NULL AFTER `troco_para`
                    ");
                }
            ...
            $s = $pdo->prepare("
                INSERT INTO pedidos (cliente_id, total, status, metodo_pagamento, parcelas, troco_para, observacao)
                VALUES (?, ?, 'pendente', ?, ?, ?, ?)
            ");
            $s->execute([clienteId(), $total, $metodo, $parcelas, $troco, $obs ?: null]);
        ...
        case 'meus_pedidos':
            ...
            $s = $pdo->prepare("
                SELECT p.id, p.status, p.total, p.criado_em,
                       COALESCE(p.metodo_pagamento, '')  AS metodo_pagamento,
                       COALESCE(p.parcelas, 1)           AS parcelas,
                       p.observacao,
