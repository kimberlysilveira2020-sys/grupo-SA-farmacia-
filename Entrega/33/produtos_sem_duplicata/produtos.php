...
// Consulta produtos — mostra só o produto ativo de maior ID por nome+fabricante (deduplica)
$stmt = $pdo->query("
    ...
    WHERE COALESCE(p.ativo, 1) = 1
      AND NOT EXISTS (
          ...
          WHERE p2.nome = p.nome
            AND p2.fabricante = p.fabricante
            AND COALESCE(p2.ativo, 1) = 1
            AND p2.id > p.id
      )
    LEFT JOIN (
        SELECT produto_id,
...
<script>
    ...
    document.getElementById('formNovoProduto').addEventListener('submit', async function(event) {
        ...
        try {
            ...
            if (result.success) {
                // Fecha modal e navega sem depender de evento Bootstrap
                const modalEl = document.getElementById('modalNovoProduto');
                const bsModal = bootstrap.Modal.getInstance(modalEl);
                if (bsModal) bsModal.hide();
                // Delay curto para modal fechar antes de navegar
                setTimeout(() => {
